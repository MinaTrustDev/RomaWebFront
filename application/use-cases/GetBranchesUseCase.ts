...
export class GetBranchesUseCase {
  ...
  async execute(orderType: string): Promise<BranchDTO[]> {
    ...
    console.log("GetBranchesUseCase.execute - orderType:", orderType, "-> orderTypeParam:", orderTypeParam);
    const branches = await this.branchRepository.getBranchesByOrderType(orderTypeParam);
